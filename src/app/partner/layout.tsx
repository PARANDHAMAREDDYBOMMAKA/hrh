...
export default function PartnerLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (status === "authenticated" && session?.user?.role !== "PARTNER") router.push("/");
    ...
    <div className="min-h-screen flex bg-[#fafaf9]">
      <aside className={`fixed inset-y-0 left-0 z-50 w-65 bg-white border-r border-black/4 transform transition-transform duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="p-7 pb-5">
          <Link href="/partner/dashboard" className="flex items-center gap-2.5 group">
            ...
            <div>
              ...
              <span className="block text-[10px] text-foreground/30 font-semibold tracking-widest uppercase">Partner</span>
            </div>
          </Link>
        </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4">
          ...
          <button onClick={() => signOut({ callbackUrl: "/" })} className="flex items-center gap-2 px-3.5 py-2.5 text-[13px] text-foreground/30 hover:text-red-500 transition-colors duration-300 w-full rounded-xl hover:bg-red-50/50">
